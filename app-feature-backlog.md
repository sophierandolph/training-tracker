...
- [ ] **Pre-commit hook is missing entirely (discovered Aug 2026)** -- `.git/hooks/pre-commit` doesn't exist post machine-migration (git hooks aren't version-controlled, so it didn't survive the Jul 30 move to the new Mac). Nothing in the repo to restore it from. Until rebuilt, the 3 parallel deploy review agents are the only syntax safety net -- fine for now but worth reinstalling properly, ideally with a real JS parser (`node --check`) instead of the old Python regex approach, which also had a known gap: it passed on `'that's'` and similar unescaped apostrophes in single-quoted strings, causing silent JS SyntaxErrors that blank out the app. Short-term workaround unchanged: always use double quotes for any string containing an apostrophe/contraction.
- [ ] **Autosave key scoped by workout name, not date** -- `autosaveProgress()` keys localStorage as `'workoutInProgress_' + workout.name`, with no date component. `restoreProgressIfAvailable()` checks the saved date against today so it won't *display* the wrong day's draft, but it doesn't stop an unfinished session from being *overwritten*: starting a same-named workout again on a later date silently clobbers any earlier abandoned draft under that key. Only bites if a session is started and never finished (no "Finish Workout" tap). Newly relevant as of Aug 2026 -- the return-to-training plan reuses the same 3 fixed template objects (same names) across multiple dates on purpose, so this path is now exercised weekly instead of rarely. Fix: include the date in the autosave key.
- [ ] **Localhost preview shows Feb 8 instead of today** -- the localhost auth bypass (added May 28) skips sign-in, so Firestore reads fail (security rules require auth). Without Firestore, the app has no readiness data and `getActiveWeeks()` likely falls back to the first week in WEEK_DATES (Feb 9 week → renders from Feb 8). Used to work because Firebase Auth persisted the session across localhost visits. Fix options: (a) restore the sign-in-with-popup flow on localhost and debug the loop, or (b) on localhost, attempt `onAuthStateChanged` passively and only show the gate if truly not signed in (don't force popup). The bypass-without-Firestore pattern doesn't work.
...
- [ ] **dashboard.html stale 130/150 references after Apr 27 protein target bump (135/155)** — chart reference lines, fallback constants, stale comment, and tooltip threshold. Specifically: chart reference lines at lines ~1638-1649 (`[130, 150]` and `val === 150` checks) → 135/155; fallback `f.target || 130` at lines 926, 931, 1018, 1596, 1610, 1616 → `|| 135`; stale comment at line 1594 (`130 rest day, 150 training day`) → 135/155; threshold `target >= 150 ? 'Training day' : 'Rest day'` at line 1700 still functions correctly for all four values but reads as outdated. Non-breaking (per-log targets stamped at save-time keep historical adherence accurate). Bundle as one dashboard cleanup pass.
