...
- [ ] **Pre-commit hook doesn't catch unescaped apostrophes in JS strings** -- Python regex checker passes on `'that's'` and similar contractions in single-quoted strings; these cause silent JS SyntaxErrors that blank out the app. Fix: use a real JS parser (e.g. `node --check`) in the hook instead of Python regex. Short-term workaround: always use double quotes for any string containing an apostrophe/contraction.
